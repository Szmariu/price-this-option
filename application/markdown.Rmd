...
A Monte Carlo approach is necessary, because it is not possible to determine the price analitically. 
There exist some solutions that will give aproximate prices, but only Monte Carlo will
find them with aribrary precision (*theoritically*, given enough computational power). 
...
A good target for this strategy are volitile stocks, such as *Tesla*, 
that can swing in both directions. *Bitcoin* would be another usefull example, but the market for 
...
of the price of the underlying asset, not on the last price. They naturally lower the volatilility 
...
even given that the time to matuirty is one year. This is an effect of the
direct opposite effect of the geometric asian options, that lower volatility 
and long strangle strategy, that favors high volatiliy. 
